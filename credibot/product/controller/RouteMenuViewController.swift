...
class RouteMenuViewController: BaseViewController {
...
extension RouteMenuViewController {
    
    private func saveInfo(to dict: [String: String]) async {
        KRProgressHUD.show(withMessage: "loading...")
        let man = NetworkManager()
        do {
            let result = try await man.request(.postData(endpoint: "/cbd/began", parameters: dict), responseType: BaseModel.self)
    ...
    private func getListDetailInfo() async {
        ...
        do {
            let result = try await man.request(.postData(endpoint: "/cbd/peoplethey", parameters: dict), responseType: BaseModel.self)
            let wanted = result.wanted ?? ""
            if wanted == "0" || wanted == "00" {
